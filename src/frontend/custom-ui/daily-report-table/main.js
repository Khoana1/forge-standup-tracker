...
const emitResize = () => {
  ...
  requestAnimationFrame(() => {
    ...
    events.emit('DAILY_TABLE_RESIZE', { height });
  });
};
...
const emitChange = () => {
  events.emit('DAILY_TABLE_CHANGE', {
  ...
  return `
    <div class="daily-table-wrap">
      ...
      <div class="daily-table-footer">
        ...
        ${atMax ? `<p class="footer-hint">Tối đa ${state.maxRows} hàng.</p>` : ''}
      </div>
    </div>
  `;
};
...
if (root) {
  ...
  root.addEventListener(
    ...
    (event) => {
      const target = event.target;
      if (!target.dataset?.rowId || !target.dataset?.field) return;
      events.emit('DAILY_TABLE_CELL_BLUR', {
  ...
  root.addEventListener('click', async (event) => {
    ...
    if (target.id === 'add-row') {
      events.emit('DAILY_TABLE_ADD_ROW');
      return;
    }

    if (target.dataset.removeRow) {
      events.emit('DAILY_TABLE_REMOVE_ROW', { rowId: target.dataset.removeRow });
      return;
    }
...
events.on('DAILY_TABLE_PAUSE', () => {
  isPaused = true;
  closeAddModal();
});

events.on('DAILY_TABLE_SYNC', (payload) => {
  isPaused = false;
