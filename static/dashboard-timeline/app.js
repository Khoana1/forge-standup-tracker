...
// src/frontend/custom-ui/dashboard-timeline/main.js
var import_bridge = __toESM(require_out3());
var root = document.getElementById("root");
var AVATAR_COLORS = ["#0c66e4", "#1f845a", "#e56910", "#8270db", "#c9372c", "#5b7f24"];
var state = {
  entries: [],
...
var memberInitials = (name) => (name ?? "?").split(/\s+/).map((w) => w[0]).join("").slice(0, 2).toUpperCase();
var avatarColor = (accountId) => {
  const s = accountId ?? "";
  let h = 0;
...
var statusCardHtml = (variant, label, text) => {
  if (!text?.trim()) return "";
  return `
    ...
    <article class="entry-card" data-account-id="${escapeHtml(entry.accountId)}">
      ...
      ${issues.length ? `
        <section class="work-items-section">
          <p class="work-items-title">Work item li\xEAn k\u1EBFt</p>
      ...
      ${entry.hasBlocker && state.canAdminister ? `<button type="button" class="btn-resolve" data-resolve="${escapeHtml(entry.accountId)}">\u0110\xE1nh d\u1EA5u \u0111\xE3 x\u1EED l\xFD v\u1EA5n \u0111\u1EC1</button>` : ""}
    </article>
  `;
};
var emitResize = () => {
  requestAnimationFrame(() => {
    ...
    import_bridge.events.emit("DASHBOARD_TIMELINE_RESIZE", { height });
  });
};
var render = () => {
  if (!root) return;
...
if (root) {
  root.addEventListener("click", (event) => {
    ...
    if (resolveBtn?.dataset.resolve) {
      import_bridge.events.emit("DASHBOARD_TIMELINE_RESOLVE", { accountId: resolveBtn.dataset.resolve });
    }
  });
}
import_bridge.events.on("DASHBOARD_TIMELINE_SYNC", (payload) => {
...
render();
import_bridge.events.emit("DASHBOARD_TIMELINE_READY");
