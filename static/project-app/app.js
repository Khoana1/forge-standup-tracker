...
var enrichIssues = async (issues) => {
  ...
  try {
    const result = await (0, import_bridge2.invoke)("enrichLinkedIssues", { issueKeys: keys });
...
async function renderDashboard(container, ctx2) {
    ...
    <div class="page dashboard-page">
      ...
      ${members.length ? `<section class="section">
              ...
              <div class="member-grid">
                ${members.map(
    (m) => `
                  <button type="button" class="member-card${ctx2.memberFilter === m.accountId ? " is-selected" : ""}" data-member="${escapeHtml(m.accountId)}">
  ...
  container.querySelector("#export-data")?.addEventListener("click", async () => {
    try {
      ...
      alert(`\u0110\xE3 chu\u1EA9n b\u1ECB xu\u1EA5t ${payload.entryCount} b\u1EA3n ghi.`);
    } catch (e) {
      alert(e?.message ?? "Kh\xF4ng xu\u1EA5t \u0111\u01B0\u1EE3c d\u1EEF li\u1EC7u.");
    }
  });
