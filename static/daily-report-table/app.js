...
`);
    (function(undefined2) {
      ...
      var count = 0, destroyObserver, logEnabled = false, hiddenCheckEnabled = false, msgHeader = "message", msgHeaderLen = msgHeader.length, msgId = "[iFrameSizer]", msgIdLen = msgId.length, pagePosition = null, requestAnimationFrame2 = window.requestAnimationFrame, resetRequiredMethods = Object.freeze({
        max: 1,
        scroll: 1,
        bodyScroll: 1,
...
// src/frontend/custom-ui/daily-report-table/main.js
...
var state = {
  rows: [],
  placeholders: { tasks: "", progress: "", problems: "" },
  headers: { tasks: "Tasks", progress: "Progress", problems: "Problems" },
  minRows: 1,
  maxRows: 20,
  showIssues: false,
  issuesTitle: "Work item l\xE0m vi\u1EC7c h\xF4m nay",
  projectKey: "",
  issues: [],
  lockedKeys: [],
  maxIssues: 20,
...
var syncAllRowHeights = () => {
  root?.querySelectorAll("tr[data-row-id]").forEach(syncRowHeights);
};
var MODAL_MIN_IFRAME_HEIGHT = 580;
var emitResize = () => {
  if (isPaused) return;
  requestAnimationFrame(() => {
    ...
    import_bridge.events.emit("DAILY_TABLE_RESIZE", { height });
  });
};
var scheduleResize = () => {
  emitResize();
...
var emitChange = () => {
  import_bridge.events.emit("DAILY_TABLE_CHANGE", {
...
var rowHtml = (row, index) => {
  const canRemove = state.rows.length > state.minRows;
  return `
    <tr data-row-id="${escapeHtml(row.id)}">
      <td class="cell-index">${index + 1}</td>
      <td class="cell-content">
        <textarea
          class="cell-input cell-input-tasks"
          data-row-id="${escapeHtml(row.id)}"
          data-field="tasks"
          rows="1"
          placeholder="${escapeHtml(state.placeholders.tasks)}"
        >${escapeHtml(row.tasks ?? "")}</textarea>
      </td>
      <td class="cell-content">
        <input
          type="text"
          class="cell-input cell-input-progress"
          data-row-id="${escapeHtml(row.id)}"
          data-field="progress"
          value="${escapeHtml(row.progress ?? "")}"
          placeholder="${escapeHtml(state.placeholders.progress)}"
          title="${escapeHtml(row.progress ?? "")}"
        />
      </td>
      <td class="cell-content">
        <textarea
          class="cell-input cell-input-problems"
          data-row-id="${escapeHtml(row.id)}"
          data-field="problems"
          rows="1"
          placeholder="${escapeHtml(state.placeholders.problems)}"
        >${escapeHtml(row.problems ?? "")}</textarea>
      </td>
      <td class="cell-actions">
        <button
          type="button"
          class="btn-row-remove"
          data-remove-row="${escapeHtml(row.id)}"
          title="X\xF3a h\xE0ng"
          ${canRemove ? "" : "disabled"}
        >\xD7</button>
      </td>
    </tr>
...
var issueRowHtml = (issue, index) => {
  const status = getStatusDisplay(issue);
  const locked = state.lockedKeys.includes(issue.key);
  const canRemove = !locked;
  const dragAttrs = state.enableReorder ? `draggable="true" data-drag-index="${index}"` : "";
  return `
    <div class="row-card" data-key="${escapeHtml(issue.key)}" data-index="${index}" role="listitem">
      ${state.enableReorder ? `<span class="grip" ${dragAttrs} aria-hidden="true" title="K\xE9o \u0111\u1EC3 s\u1EAFp x\u1EBFp">\u283F</span>` : ""}
...
var renderTableSection = () => {
  const atMax = state.rows.length >= state.maxRows;
  return `
    <div class="daily-table-wrap">
      <table class="daily-table" role="grid">
        <colgroup>
          ...
          <col class="col-problems" />
          <col class="col-actions" />
        </colgroup>
        <thead>
          <tr>
            <th scope="col">#</th>
            <th scope="col">${escapeHtml(state.headers.tasks)}</th>
            <th scope="col">${escapeHtml(state.headers.progress)}</th>
            <th scope="col">${escapeHtml(state.headers.problems)}</th>
            <th scope="col" aria-label="Thao t\xE1c"></th>
          </tr>
        </thead>
        <tbody>
          ${state.rows.map((row, index) => rowHtml(row, index)).join("")}
        </tbody>
      </table>
      <div class="daily-table-footer">
        <button type="button" class="btn-add-row" id="add-row" ${atMax ? "disabled" : ""}>
          Th\xEAm h\xE0ng
        </button>
        ${atMax ? `<p class="footer-hint">T\u1ED1i \u0111a ${state.maxRows} h\xE0ng.</p>` : ""}
      </div>
    </div>
  `;
};
    ...
    <div class="modal-backdrop" data-modal-backdrop role="presentation">
      <div class="modal-dialog" role="dialog" aria-modal="true" aria-labelledby="add-work-item-title">
        <div class="modal-header">
          <h4 class="modal-title" id="add-work-item-title">Th\xEAm work item</h4>
        ...
        <div class="modal-body">
          ...
          </div>` : `<p class="error">\u0110\xE3 \u0111\u1EA1t t\u1ED1i \u0111a ${state.maxIssues} work item.</p>`}
          ${state.projectKey && !atLimit ? `
          <div class="panel">
            <label class="panel-label" for="search-input">T\xECm work item trong project</label>
            <div class="search-row">
              <input id="search-input" class="search-input" type="text" placeholder="M\xE3 work item ho\u1EB7c t\u1EEB kh\xF3a\u2026" value="${escapeHtml(state.searchQuery)}" />
              <button type="button" class="btn" id="search-btn" ${state.searching ? "disabled" : ""}>${state.searching ? "\u2026" : "T\xECm"}</button>
            </div>
            ${state.suggestions.length ? `<div class="suggestions">${state.suggestions.map(
  ...
  return `
    <section class="issues-section">
      ${hasIssues ? `
        ...
        ${!atLimit ? `<div class="issues-section-footer">
                <button type="button" class="btn btn-primary" id="open-add-modal">Th\xEAm work item</button>
              </div>` : ""}` : `
...
var render = () => {
  if (!root) return;
  root.innerHTML = `
    <div class="log-form${state.addModalOpen ? " is-modal-open" : ""}">
      ${renderTableSection()}
      ${state.showIssues ? renderIssuesSection() : ""}
...
var updateField = (rowId, field, value) => {
  const row = state.rows.find((item) => item.id === rowId);
  if (!row || !field) return;
  row[field] = value;
  emitChange();
};
var reorderIssues = (fromIndex, toIndex) => {
  if (fromIndex === toIndex || fromIndex == null || toIndex == null) return;
  const next = [...state.issues];
  const [moved] = next.splice(fromIndex, 1);
  next.splice(toIndex, 0, moved);
  import_bridge.events.emit("ISSUE_HUB_REORDER", { keys: next.map((issue) => issue.key) });
};
if (root) {
  root.addEventListener("input", (event) => {
    ...
    if (target.id === "paste-input") {
      ...
      if (addBtn) {
        ...
        addBtn.textContent = count > 1 ? `Th\xEAm ${count} work item` : "Th\xEAm work item";
      }
      return;
    }
    ...
    updateField(target.dataset.rowId, target.dataset.field, target.value);
    if (target.dataset.field === "progress") {
      target.title = target.value;
    }
    if (target.tagName === "TEXTAREA") {
      syncRowHeights(target.closest("tr"));
      emitResize();
    }
  });
  root.addEventListener(
    ...
    (event) => {
      const target = event.target;
      if (!target.dataset?.rowId || !target.dataset?.field) return;
      import_bridge.events.emit("DAILY_TABLE_CELL_BLUR", {
  ...
  root.addEventListener("click", async (event) => {
    ...
    if (openEl?.dataset.open) {
      ...
      import_bridge.events.emit("ISSUE_HUB_OPEN", { key: openEl.dataset.open });
      return;
    }
    ...
    if (target.id === "add-row") {
      import_bridge.events.emit("DAILY_TABLE_ADD_ROW");
      return;
    }
    if (target.dataset.removeRow) {
      import_bridge.events.emit("DAILY_TABLE_REMOVE_ROW", { rowId: target.dataset.removeRow });
      return;
    }
    if (target.id === "paste-add") {
      const found = parsePaste(state.pasteText);
      if (!found.length) {
        state.pasteError = "Ch\u01B0a nh\u1EADn di\u1EC7n \u0111\u01B0\u1EE3c link Jira ho\u1EB7c m\xE3 work item.";
        render();
        return;
      }
      const room = state.maxIssues - state.issues.length;
      if (room <= 0) {
        state.pasteError = `\u0110\xE3 \u0111\u1EA1t t\u1ED1i \u0111a ${state.maxIssues} work item.`;
        render();
        return;
      }
      import_bridge.events.emit("ISSUE_HUB_ADD", { items: found.slice(0, room) });
      closeAddModal();
      render();
      return;
    }
    if (target.id === "search-btn") {
      ...
      if (pasted.length === 1) {
        import_bridge.events.emit("ISSUE_HUB_ADD", { items: pasted.slice(0, 1) });
        closeAddModal();
        render();
        return;
      }
      ...
      render();
      try {
        const result = await (0, import_bridge.invoke)("searchIssuesForLink", { projectKey: state.projectKey, query });
    ...
    if (target.dataset.removeIssue) {
      ...
      setTimeout(() => import_bridge.events.emit("ISSUE_HUB_REMOVE", { key: target.dataset.removeIssue }), 200);
      return;
    }
    if (target.dataset.status && state.enableStatusChange) {
      ...
      import_bridge.events.emit("ISSUE_HUB_STATUS_CHANGE", { issueKey: issue.key, nextStatus: next });
      return;
    }
    if (target.dataset.add) {
      if (state.issues.some((issue) => issue.key === target.dataset.add)) return;
      import_bridge.events.emit("ISSUE_HUB_ADD", {
        items: [
          {
...
import_bridge.events.on("DAILY_TABLE_PAUSE", () => {
  isPaused = true;
  closeAddModal();
});
import_bridge.events.on("DAILY_TABLE_SYNC", (payload) => {
  isPaused = false;
  ...
  state.placeholders = {
    tasks: payload?.placeholders?.tasks ?? "",
    progress: payload?.placeholders?.progress ?? "",
    problems: payload?.placeholders?.problems ?? ""
  };
  state.headers = {
    tasks: payload?.headers?.tasks ?? "Tasks",
    progress: payload?.headers?.progress ?? "Progress",
    problems: payload?.headers?.problems ?? "Problems"
  };
  state.minRows = payload?.minRows ?? 1;
  state.maxRows = payload?.maxRows ?? 20;
  render();
});
import_bridge.events.on("ISSUE_HUB_SYNC", (payload) => {
  ...
  state.issuesTitle = payload?.title ?? "Work item l\xE0m vi\u1EC7c h\xF4m nay";
  render();
});
render();
import_bridge.events.emit("DAILY_TABLE_READY");
import_bridge.events.emit("ISSUE_HUB_READY");
